...
const PaymentGrid: React.FC<{ className?: string; theme?: 'bw' }> = ({
  ...
  const [selectedGateways, setSelectedGateways] = useState<PaymentGateway[]>(
    []
  );
  ...
  > = {
    ...
    CASH_ON_DELIVERY: {
      name: t('common:payment-cash-on-delivery'),
  ...
  useEffect(() => {
    if (Array.isArray(settings?.paymentGateway)) {
      const gateways = settings?.paymentGateway.map(
        (g) => g.name.toUpperCase() as PaymentGateway
      );
      if (gateways) {
    ...
    <div className={className}>
      ...
      <RadioGroup value={gateway} onChange={setGateway}>
        ...
        <div className="mb-8 grid grid-cols-2 gap-4 md:grid-cols-3">
          {settings?.paymentGateway?.map((gateway: { name: string }) => {
            let key = gateway.name.toUpperCase();
            return (
              <PaymentGroupOption
                key={key}
                theme={theme}
                payment={AVAILABLE_PAYMENT_METHODS_MAP[key as PaymentGateway]}
              />
            );
          })}
        </div>
